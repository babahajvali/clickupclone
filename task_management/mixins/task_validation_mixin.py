...
class TaskValidationMixin:
    ...
    def check_task_is_not_deleted(self, task_id: str):
        task_data = self.validate_task_is_exists(task_id=task_id)

        is_task_deleted = task_data.is_deleted
        if is_task_deleted:
            raise DeletedTaskFound(task_id=task_id)

    def validate_task_is_exists(self, task_id: str) -> TaskDTO:
