...
class TestDeleteSpaceInteractor:
    def setup_method(self):
        self.space_storage = create_autospec(SpaceStorageInterface)
        self.workspace_storage = create_autospec(WorkspaceStorageInterface)

        self.interactor = DeleteSpaceInteractor(
    ...
    def _setup_dependencies(self, role: Role = Role.MEMBER):
        ...
        self.space_storage.delete_space.return_value = make_space()

    def test_delete_space_success(self, snapshot):
        self._setup_dependencies()

        result = self.interactor.delete_space(space_id="space_1", deleted_by="user_1")

        snapshot.assert_match(repr(result), "delete_space_success.txt")
    ...
    def test_delete_space_not_found(self, snapshot):
        ...
        with pytest.raises(SpaceNotFound) as exc:
            self.interactor.delete_space(space_id="space_1", deleted_by="user_1")

        snapshot.assert_match(repr(exc.value), "delete_space_not_found.txt")

    def test_delete_space_permission_denied(self, snapshot):
        ...
        snapshot.assert_match(
            repr(exc.value), "delete_space_permission_denied.txt"
        )
