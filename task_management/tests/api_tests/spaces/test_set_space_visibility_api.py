...
from task_management.tests.api_tests.spaces import BaseSetSpaceVisibility
...
def update_space_visibility_mock(mocker):
    return mocker.patch(
        "task_management.storages.space_storage.SpaceStorage.update_space_visibility"
    )
...
def make_visibility_space_output():
    return type(
        "SpaceOutput",
        (),
        {
...
class TestSetSpaceVisibilityAPI(BaseSetSpaceVisibility):
    ...
    def test_set_space_visibility_successfully(self, snapshot, mocker):
        self._setup_common(mocker)

        update_visibility = update_space_visibility_mock(mocker)
        update_visibility.return_value = make_visibility_space_output()
    ...
    def test_set_space_visibility_not_found(self, snapshot, mocker):
        get_space = get_space_mock(mocker)
    ...
    def test_set_space_visibility_permission_denied(self, snapshot, mocker):
