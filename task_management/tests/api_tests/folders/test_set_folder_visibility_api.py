...
from task_management.tests.api_tests.folders import BaseSetFolderVisibility
...
def update_folder_visibility_mock(mocker):
    return mocker.patch(
        "task_management.storages.folder_storage.FolderStorage.update_folder_visibility"
    )
...
class TestSetFolderVisibilityAPI(BaseSetFolderVisibility):
    ...
    def test_set_folder_visibility_successfully(self, snapshot, mocker):
        self._setup_common(mocker)

        update_visibility = update_folder_visibility_mock(mocker)
        update_visibility.return_value = FolderDTO(
            folder_id="folder_1",
            name="Product",
            description="Product folder",
            space_id="space_1",
    ...
    def test_set_folder_visibility_not_found(self, snapshot, mocker):
        get_folder = get_folder_mock(mocker)
    ...
    def test_set_folder_visibility_permission_denied(self, snapshot, mocker):
