...
class TaskAssigneeInteractor(ValidationMixin):
    ...
    def assign_task_assignee(self, task_id: str, user_id: str,assigned_by) -> TaskAssigneeDTO:
        self.validate_user_exist_and_status(user_id=user_id, user_storage=self.user_storage)
        list_id = self.validate_task_exists(task_id=task_id,task_storage=self.task_storage)
        self.check_user_has_access_to_list_modification(user_id=user_id,list_id=list_id,permission_storage=self.permission_storage)
